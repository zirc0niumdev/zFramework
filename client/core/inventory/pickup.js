...
onNet('Client.Pickup.CreatePickup', (action, item) => {
    switch (action) {
...
async function spawnPickup(object) {
    ...
    .then(hasLoaded => {
        if (hasLoaded) {
            ...
            if (pos.rot) SetEntityRotation(handle, pos.rot.x, pos.rot.y, pos.rot.z);
        ...
        if (myPickups && myPickups.length > 0) {
            ...
            for (const [_, object] of Object.entries(myPickups)) {
                ...
                if (zFramework.GetDistanceByCoords(objPos, playerPos) < 2) {
                    zFramework.Functions.DrawText3D(objPos.x, objPos.y, objPos.z + 0.25, "[F] pour ~p~ramasser");
                    break;
                }
            }
        }
